class PoliciesPage{

    get search_policy_txt(){
        return browser.element("//input[@placeholder='Search Policies']")
    }

    get_policy_element(policy_name) {
        return browser.element(`//div[contains(text(),' ${policy_name} ')]`);
    }

    get_remove_policy_btn(policy_name) {
        let tmp_del_locator = "//div[contains(text(),' $POLICY ')]/ancestor::div[@class='main-row row tr']/div[contains" +
            "(@class,'policy-cell-middle policy-cell-actions')]/i[@title='Remove policy']";
        return browser.element(tmp_del_locator.replace('$POLICY', policy_name));
    }

    click_remove_policy(policy_to_del) {
        this.get_remove_policy_btn(policy_to_del).waitForVisible(5 * 1000);
        this.get_remove_policy_btn(policy_to_del).click();
    }

    search_policy(policy_name){
        this.search_policy_txt.waitForVisible(20 * 1000);
        this.search_policy_txt.setValue(policy_name);
    }
}

module.exports = new PoliciesPage();
